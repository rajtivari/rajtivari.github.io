...
export default function WireGrid() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let j = 0; j <= rows; j++) {
        ctx.beginPath();
        const fade = 0.32 - Math.abs(j - rows / 2) / rows * 0.22;
        ctx.strokeStyle = `rgba(62,123,250,${Math.max(0.05, fade)})`;
        for (let i = 0; i <= cols; i++) {
          const [x, y] = pts[j][i];
      ...
      for (let i = 0; i <= cols; i++) {
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(62,123,250,0.10)';
        for (let j = 0; j <= rows; j++) {
          const [x, y] = pts[j][i];
      ...
      if (mouse.current.active) {
        ctx.beginPath();
        ctx.fillStyle = 'rgba(62,123,250,0.9)';
